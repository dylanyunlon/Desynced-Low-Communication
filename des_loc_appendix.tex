...
&\leq -\frac{\eta}{2}\mathbb{E}\|\nabla f(x_t)\|^2
  -\frac{\eta}{2}\mathbb{E}\left\|\frac{1}{M}\sum_{m=1}^{M}\nabla f_m(x_t^m)\right\|^2
  +\frac{\eta L^2}{2M}\sum_{m=1}^{M}\underbrace{\mathbb{E}\|x_t - x_t^m\|^2}_{\text{Lemma~\ref{lem:drift}}}.
\end{align*}
...
&\leq \frac{\eta\rho L^2}{2}\underbrace{\mathbb{E}\|z_t - x_t\|^2}_{\text{Lemma~\ref{lem:virtual_drift}}}
  + \frac{\eta}{2\rho}\mathbb{E}\left\|\frac{1}{M}\sum_{m=1}^{M}\nabla f_m(x_t^m)\right\|^2.
\end{align*}
...
&\leq -\frac{\eta}{2}\mathbb{E}\|\nabla f(x_t)\|^2
  -\frac{\eta}{2}\mathbb{E}\left\|\frac{1}{M}\sum_{m=1}^{M}\nabla f_m(x_t^m)\right\|^2
  +\frac{\eta L^2}{2M}\sum_{m=1}^{M}\underbrace{\mathbb{E}\|x_t - x_t^m\|^2}_{\text{Lemma~\ref{lem:drift}}} \\
&\quad +\frac{\eta\rho L^2}{2}\underbrace{\mathbb{E}\|z_t - x_t\|^2}_{\text{Lemma~\ref{lem:virtual_drift}}}
  +\frac{\eta}{2\rho}\mathbb{E}\left\|\frac{1}{M}\sum_{m=1}^{M}\nabla f_m(x_t^m)\right\|^2 \\
&\quad +\frac{\eta^2 L}{2M}\sigma^2
  + \frac{\eta^2 L}{2}\mathbb{E}\left\|\frac{1}{M}\sum_{m=1}^{M}\nabla f_m(x_t^m)\right\|^2.
\end{align*}

\paragraph{Step 4 (final).} Now we average over the iterates and apply the bounds derived in Lemmas~\ref{lem:virtual_drift},~\ref{lem:drift}.
\begin{align*}
...
&\quad + \frac{\eta\rho L^2}{2}\underbrace{\frac{1}{T}\sum_{t=0}^{T-1}\mathbb{E}\|z_t - x_t\|^2}_{\text{Lemma~\ref{lem:virtual_drift}}}
  + \frac{\eta L^2}{2M}\underbrace{\frac{1}{TM}\sum_{t=0}^{T-1}\sum_{m=1}^{M}\mathbb{E}\|x_t - x_t^m\|^2}_{\text{Lemma~\ref{lem:drift}}}
...
12\eta^2 L^2\psi &\leq \frac{1}{2} & &\Longleftrightarrow \quad \text{from Lemma~\ref{lem:drift}}
\end{align*}
Note that
\[
...
% NOTE for Claude-13: Continue from here with Algorithm 5 (DES-LOC-Adam probabilistic),
% virtual iterates z_t for Adam, the full z_{t+1}-z_t derivation with U_t/V_t terms,
% Step 2 (smoothness), Steps 3a-3e, and the summary bounds.
% Corresponds to PDF pages 36-39.
