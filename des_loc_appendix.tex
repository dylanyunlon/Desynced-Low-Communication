...
chose to operate at large batch sizes because adaptive optimizers provide benefits primarily in large-batch training regimes~\citep{kunstner2023noise}.
...
the final $T_{\text{DECAY}} = 10\% \times T$ steps~\citep{hagele2024scaling}.
...
\begin{enumerate}
    ...
    the synchronization period to an intermediate value of $K = 64$.
\end{enumerate}

\begin{table}[h]
\centering
\caption{Optimal learning rates $\eta^*$ for $\beta_1, \beta_2$ configurations of ADOPT/Adam.}
\label{tab:lr_sweep}
\begin{tabular}{llll}
\toprule
